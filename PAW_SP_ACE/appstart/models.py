...
class Usuario(models.Model):    
    rol = models.CharField('Boleta', max_length=45,blank = False)
...
class Materia(models.Model):
    ...
    nombre = models.CharField('Materia', max_length=45, blank=False)
    carga = models.IntegerField('Carga', null = False)

class ETS(models.Model):    
    turno = models.BinaryField()
    precio = models.DecimalField('Precio', max_digits=2, decimal_places=2)
    materia =  models.ForeignKey(Materia, on_delete = models.CASCADE) #Many to one

class Alumno_ETS(models.Model):
    alumno = models.ForeignKey(Alumno, on_delete = models.CASCADE) #Many to one
    ets = models.ForeignKey(ETS, on_delete = models.CASCADE) #Many to one
...
class Tramite(models.Model):
    alumno = models.ForeignKey(Alumno, on_delete = models.CASCADE) #Many to one
    tipo_tramite = models.ForeignKey(Tipo_tramite, on_delete = models.CASCADE) #Many to one
...
class Tipo_archivo(models.Model):
    nombre = models.CharField('Tipo de archivo', max_length=45, blank = False)

class Archivo_adjunto(models.Model):
    tramite = models.ForeignKey(Tramite, on_delete = models.CASCADE) #Many to one
    tipo_archivo = models.ForeignKey(Tipo_archivo, on_delete = models.CASCADE) #Many to one
    documento = models.BinaryField()
